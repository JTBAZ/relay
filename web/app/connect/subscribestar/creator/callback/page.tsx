...
function redirectUriForExchange(): string {
  ...
  return o
    ? `${o}/connect/subscribestar/creator/callback`
    : `${window.location.origin}/connect/subscribestar/creator/callback`;
}
...
function CallbackInner() {
  ...
  useEffect(() => {
    ...
    if (!code || !state) {
      setStatus("error");
      setMessage("Missing code or state. Start from /connect/subscribestar/connect/creator.");
      return;
    }
    ...
    (async () => {
      try {
        ...
        if (prepared) {
          if (!storedCreatorId) {
            throw new Error(
              "Missing relay_creator_id in localStorage — run Create workspace on /connect/subscribestar/connect/creator first."
            );
          }
