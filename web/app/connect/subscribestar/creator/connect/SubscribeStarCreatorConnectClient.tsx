...
export default function SubscribeStarCreatorConnectClient({ initialClientId }: Props) {
  ...
  const redirectUri = useMemo(() => {
    ...
    return envRedirect || (origin ? `${origin}/connect/subscribestar/creator/callback` : "");
  }, [origin]);
      ...
      {!legacyConnect && (
        <div className="space-y-3 rounded border border-stone-600/80 bg-stone-900/40 p-4 text-sm">
          ...
          <p className="text-xs text-stone-500">
            Uses <code className="rounded bg-stone-800 px-1">POST /auth/connect/subscribestar/creator/prepare</code>{" "}
          ...
          <p className="text-xs text-stone-500">
            ...
            <code className="rounded bg-stone-800 px-1">
              POST /api/v1/connect/subscribestar/creator/sync/posts
            </code>
            {" "}
