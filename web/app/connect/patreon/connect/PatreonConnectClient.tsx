...
export default function PatreonConnectClient({ initialClientId }: Props) {
  ...
  const redirectUri = useMemo(() => {
    ...
    return envRedirect || (origin ? `${origin}/connect/patreon/callback` : "");
  }, [origin]);
  ...
  return (
    <main className="mx-auto max-w-lg space-y-6 p-8 text-stone-200">
      <p>
        ...
        {" · "}
        <Link href="/connect/patreon/cookie" className="text-amber-400 underline decoration-amber-400/60 hover:text-amber-300">
          Session Cookie
        </Link>
        {" · "}
        <Link
