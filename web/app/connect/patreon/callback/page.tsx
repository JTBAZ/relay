...
function redirectUriForExchange(): string {
  ...
  return o ? `${o}/connect/patreon/callback` : `${window.location.origin}/connect/patreon/callback`;
}
...
function CallbackInner() {
  ...
  useEffect(() => {
    ...
    if (!code || !state) {
      setStatus("error");
      setMessage("Missing code or state. Start from /connect/patreon/connect.");
      return;
    }
    ...
    (async () => {
      try {
        ...
        // Creator onboarding prepared OAuth → return to step 5 for media import
        const destination = prepared
          ? "/onboarding?path=creator&step=5"
          : "/";
        setTimeout(() => {
