...
type Props = {
  ...
  /** Patreon campaign banner URL (`image_url`); shown as a thin top accent. */
...
export default function LibraryTopBar({
  ...
  return (
    <header className="relative flex min-h-[3.75rem] shrink-0 items-center justify-between border-b border-[var(--lib-border)] bg-[var(--lib-card)] px-4 py-2">
      {campaignBannerUrl ? (
        <div
          className="pointer-events-none absolute inset-x-0 top-0 z-0 h-1 bg-cover bg-center opacity-90"
          style={{ backgroundImage: `url(${campaignBannerUrl})` }}
          aria-hidden
        />
      ) : null}
      <div className="group relative z-10 flex min-w-0 flex-1 items-center gap-3 rounded-lg px-2 py-1 hover:bg-[var(--lib-muted)]/40">
