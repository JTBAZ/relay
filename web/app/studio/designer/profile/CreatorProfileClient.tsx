...
type PostingGoalFormState = {
  monthly_post_target: number;
  bonus_nudges_enabled: boolean;
};

const DEFAULT_POSTING_GOAL_FORM: PostingGoalFormState = {
  monthly_post_target: DEFAULT_POSTS_PER_MONTH,
...
function postingGoalToForm(goal: {
  monthly_post_target: number;
  bonus_nudges_enabled: boolean;
}): PostingGoalFormState {
  return {
    monthly_post_target: clampMonthlyPostTarget(goal.monthly_post_target),
    bonus_nudges_enabled: Boolean(goal.bonus_nudges_enabled),
  };
}
...
export default function CreatorProfileClient() {
  ...
  const [postingGoalForm, setPostingGoalForm] = useState<PostingGoalFormState>(
    DEFAULT_POSTING_GOAL_FORM
  );
  const [postingGoalBaseline, setPostingGoalBaseline] = useState<PostingGoalFormState>(
    DEFAULT_POSTING_GOAL_FORM
  );
  const [postingGoalSaving, setPostingGoalSaving] = useState(false);
  const [postingGoalError, setPostingGoalError] = useState<string | null>(null);
  const [postingGoalSavedNote, setPostingGoalSavedNote] = useState<string | null>(null);

  const load = useCallback(async () => {
    setError(null);
    setPostingGoalError(null);
    setLoading(true);
    try {
      const [prof, postingGoalRes] = await Promise.all([
        getCreatorProfile(),
      ...
      setForm(next);
      setBaseline(next);
      const goalForm = postingGoalRes?.goal
        ? postingGoalToForm(postingGoalRes.goal)
        : DEFAULT_POSTING_GOAL_FORM;
      setPostingGoalForm(goalForm);
      setPostingGoalBaseline(goalForm);
    } catch (e) {
  ...
  const postingGoalDirty =
    postingGoalForm.monthly_post_target !== postingGoalBaseline.monthly_post_target ||
    postingGoalForm.bonus_nudges_enabled !== postingGoalBaseline.bonus_nudges_enabled;
  const postingGoalInvalid =
    !Number.isFinite(postingGoalForm.monthly_post_target) ||
    postingGoalForm.monthly_post_target < 1 ||
    postingGoalForm.monthly_post_target > MAX_POSTS_PER_MONTH;

  const onSavePostingGoal = async () => {
    if (!postingGoalDirty || postingGoalInvalid) return;
    setPostingGoalSaving(true);
    setPostingGoalError(null);
    setPostingGoalSavedNote(null);
    try {
      const monthlyPostTarget = clampMonthlyPostTarget(postingGoalForm.monthly_post_target);
      const { goal } = await putCreatorPostingGoal({
        monthly_post_target: monthlyPostTarget,
        bonus_nudges_enabled: postingGoalForm.bonus_nudges_enabled,
        timezone: browserTimezone(),
      });
      const refreshed = postingGoalToForm(goal);
      setPostingGoalForm(refreshed);
      setPostingGoalBaseline(refreshed);
      setPostingGoalSavedNote("Posting rhythm saved.");
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      setPostingGoalError(msg);
    } finally {
      setPostingGoalSaving(false);
    }
  };
  ...
  const publicUrl = useMemo(() => {
    ...
    if (typeof window === "undefined") {
      return `/${encodeURIComponent(identity.public_slug)}`;
    }
    const o = getWebAppOrigin() || window.location.origin;
    return `${o}/${encodeURIComponent(identity.public_slug)}`;
  }, [identity]);
  ...
  return (
    ...
    >
      ...
      {/* Posting rhythm */}
      <section className="rounded-xl border border-[var(--lib-border)] bg-[var(--lib-card)] px-4 py-4">
        <h2 className="text-sm font-semibold text-[var(--lib-fg)]">Posting rhythm</h2>
        <p className="mt-1 text-xs text-[var(--lib-fg-muted)]">
          How often you want to publish Relay-native posts each month. Relay uses this only to
          help you stay on pace — never to judge your output.
        </p>

        <div className="mt-4 flex flex-col gap-4">
          <label className="flex flex-col gap-1.5">
            <span className="text-xs font-medium text-[var(--lib-fg-muted)]">
              Relay posts per month
            </span>
            <input
              type="number"
              min={1}
              max={MAX_POSTS_PER_MONTH}
              value={postingGoalForm.monthly_post_target}
              onChange={(e) => {
                const next = Number(e.target.value);
                setPostingGoalForm((prev) => ({
                  ...prev,
                  monthly_post_target: Number.isFinite(next)
                    ? clampMonthlyPostTarget(next)
                    : DEFAULT_POSTS_PER_MONTH,
                }));
                setPostingGoalSavedNote(null);
              }}
              className="max-w-xs rounded-lg border border-[var(--lib-border)] bg-[var(--lib-bg)] px-3 py-2 text-sm text-[var(--lib-fg)] outline-none ring-[#2D6A4F]/30 focus:ring-2"
              aria-label="Relay posts per month"
              aria-invalid={postingGoalInvalid || undefined}
            />
            {postingGoalInvalid ? (
              <span className="text-[11px] text-red-400">
                Enter a whole number between 1 and {MAX_POSTS_PER_MONTH}.
              </span>
            ) : (
              <span className="text-[11px] text-[var(--lib-fg-muted)]">
                Most creators start with 1.
              </span>
            )}
          </label>

          <label className="flex items-start gap-2 text-sm text-[var(--lib-fg)]">
            <input
              type="checkbox"
              checked={postingGoalForm.bonus_nudges_enabled}
              onChange={(e) => {
                setPostingGoalForm((prev) => ({
                  ...prev,
                  bonus_nudges_enabled: e.target.checked,
                }));
                setPostingGoalSavedNote(null);
              }}
              className="mt-0.5 h-4 w-4 rounded border-[var(--lib-border)]"
            />
            <span>Suggest an extra post when I have unused media ready.</span>
          </label>
        </div>

        <div className="mt-4 flex flex-wrap items-center justify-end gap-3 border-t border-[var(--lib-border)] pt-4">
          {postingGoalError ? (
            <p role="alert" className="mr-auto text-xs text-red-400">
              {postingGoalError}
            </p>
          ) : postingGoalSavedNote ? (
            <p className="mr-auto text-xs text-[#40916C]">{postingGoalSavedNote}</p>
          ) : null}
          <button
            type="button"
            onClick={() => {
              setPostingGoalForm(postingGoalBaseline);
              setPostingGoalSavedNote(null);
              setPostingGoalError(null);
            }}
            disabled={!postingGoalDirty || postingGoalSaving}
          ...
          <button
            type="button"
            onClick={() => void onSavePostingGoal()}
            disabled={!postingGoalDirty || postingGoalSaving || postingGoalInvalid}
