...
import {
  ...
  fetchPerformanceCampaignRollups,
  fetchPerformanceGoals,
  fetchPerformanceInsightActions,
  fetchPerformanceOverview,
  fetchPerformanceTagRollups,
  fetchPerformanceWorks,
  RelayApiError,
  ...
  type PerformanceInsightActionsData,
  type PerformanceOverviewData,
  type PerformanceTagRollupsData,
  type PerformanceWorksListData
} from "@/lib/relay-api";
import { mergeInsightActionCards, type InsightActionCard } from "@/lib/merge-insight-action-cards";
...
type CreatorActionCard = InsightActionCard;
...
function latestRelayPublishDaysAgo(performance: CreatorPostPerformanceData | null): number | null {
  const published = performance?.rows
    ?.map((row) => row.relay?.published_at)
...
function strongestTierWarning(stickiness: CreatorTierStickinessData | null): string | null {
  if (!stickiness?.tiers?.length) {
    return null;
  }

  const flagged = [...stickiness.tiers]
    .filter((tier) => tier.cancel_events_in_window > 0 || tier.churn_proxy > 0)
    .sort((a, b) => {
      if (b.churn_proxy !== a.churn_proxy) return b.churn_proxy - a.churn_proxy;
  ...
  return `${flagged.title} shows ${flagged.cancel_events_in_window} cancel event${
    flagged.cancel_events_in_window === 1 ? "" : "s"
  } in this window. Treat this as a review flag, not an automated fix.`;
}

function buildCreatorActionCards({
  performance,
  summary7d,
  stickiness
}: {
  ...
  const latestDaysAgo = latestRelayPublishDaysAgo(performance);
  const hot = pickHottestFromPostPerformance(performance);
  const topInterest = performance?.rows
    ?.filter((row) => row.insights)
    .map((row) => ({
      title: row.relay?.title?.trim() || row.patreon_post_id,
      seen: row.insights?.seen ?? 0,
      impressions: row.insights?.impressions ?? 0,
      likes: row.insights?.likes ?? 0,
      comments: row.insights?.comments ?? 0
    }))
    .sort((a, b) => {
      const bScore = b.seen || b.impressions || b.likes + b.comments;
      const aScore = a.seen || a.impressions || a.likes + a.comments;
      return bScore - aScore;
    })[0];
  const tierWarning = strongestTierWarning(stickiness);
  const softMomentum = summary7d ? summary7d.net_growth_events <= 0 || summary7d.cancels_in_window > summary7d.adds_in_window : false;
  ...
  return [
    {
      ...
      trigger:
        latestDaysAgo == null
          ? "No synced Relay publish date yet."
          : `Last Relay post was ${formatDaysAgo(latestDaysAgo)}.`,
      body:
        latestDaysAgo == null || latestDaysAgo >= 7
          ? "Use stored media to get a new supporter-facing post moving without starting from a blank page."
          : "Your cadence looks recently active. Keep a saved-media post ready for the next gap.",
      actionLabel: "Create post from media storage",
      href: "/studio/new-post",
      tone: latestDaysAgo == null || latestDaysAgo >= 7 ? "active" : "watching"
    },
    {
      id: "winning-format",
      title: "Winning format",
      trigger: hot ? `${hot.title} is your current hot post.` : "Waiting for a post with Insights numbers and publish time.",
      body: hot
        ? "Repeat what is already resonating: make a follow-up with a similar format, subject, or presentation style."
        : "Upload a Patreon Insights CSV and keep posts synced so Relay can spot the format worth repeating.",
      actionLabel: "Make another post like this",
      href: "/studio/new-post",
      tone: hot ? "active" : "watching"
    },
    {
      id: "supporter-update",
      title: "Supporter momentum",
      trigger: summary7d
        ? `${summary7d.net_growth_events >= 0 ? "+" : ""}${summary7d.net_growth_events} net members over 7 days.`
        : "Waiting for membership momentum from Patreon sync.",
      body: softMomentum
        ? "Reconnect before silence turns into churn. A short studio note or work-in-progress update is enough."
        : "Momentum is stable. Keep a simple supporter update ready for the next soft patch.",
      actionLabel: "Post a supporter update",
      href: "/studio/new-post",
      tone: softMomentum ? "active" : "watching"
    },
    {
      id: "supporter-offer",
      title: "High interest, low conversion",
      trigger: topInterest
        ? `${topInterest.title} has the strongest imported interest signal.`
        : "Waiting for imported impressions or seen counts.",
      body: topInterest
        ? "If attention is not turning into paid support, improve the offer around the work: CTA placement, promo link, or a more generous Patreon discount."
        : "Once Patreon Insights are imported, Relay can point you to the post most worth attaching a stronger supporter offer to.",
      actionLabel: "Improve supporter offer",
      href: "/studio/actions",
      tone: topInterest ? "active" : "watching"
    },
    {
      id: "promo-post",
      title: "Traction worth packaging",
      trigger: hot ? `${hot.title} is getting concentrated attention.` : "Waiting for a clear traction cluster.",
      body: hot
        ? "Turn visible demand into a packaged moment: a promo post, featured drop, or campaign piece."
        : "As Relay engagement rolls up, clusters of views, favorites, snips, and comments can become promo prompts.",
      actionLabel: "Turn into promo post",
      href: "/studio/actions",
      tone: hot ? "active" : "watching"
    },
    {
      id: "tier-guidance",
      title: "Tier health warning",
      trigger: tierWarning ?? "No tier churn warning in this window.",
      body: tierWarning
        ? "Review price, content cadence, and reward timing in Patreon. Relay should flag the pattern, not pretend there is one universal fix."
        : "Tier changes are nuanced. Relay will keep this as guidance only unless there is a direct action it can safely help with.",
      tone: tierWarning ? "guidance" : "watching"
    }
  ];
}

function withDevelopmentCadenceDemo(
  performance: CreatorPostPerformanceData | null
): CreatorPostPerformanceData | null {
  if (process.env.NODE_ENV !== "development" || !performance || performance.rows.length > 0) {
    return performance;
  }
  ...
  return {
    ...
    rows: [
      {
        patreon_post_id: "dev-cadence-demo-post",
        post_id: "dev-cadence-demo-post",
        insights: {
          impressions: 840,
          seen: 420,
          likes: 38,
    ...
    relay_only_count: Math.max(performance.relay_only_count, 1),
    note: performance.note || "Development cadence demo data."
  };
}
...
function SectionCard({
  ...
  return (
    <section
      className="rounded-2xl border border-[#1F1F1F] bg-[#0D0D0D] p-4 shadow-sm sm:p-5"
    ...
    >
      ...
      {error ? (
        <p className="mt-3 rounded-xl border border-[#5c2a2a] bg-[#1a1010] px-3 py-2 text-sm text-[#f0a8a8]">
          {error}
        </p>
      ) : null}
...
export default function AnalyticsOverviewClient() {
  ...
  const [unifiedPerformance, setUnifiedPerformance] =
    useState<CreatorUnifiedPerformanceData | null>(null);
  ...
  const [performanceTags, setPerformanceTags] = useState<PerformanceTagRollupsData | null>(null);
  const [performanceWorks, setPerformanceWorks] = useState<PerformanceWorksListData | null>(null);
  const [bundleSuggestions, setBundleSuggestions] = useState<BundleSuggestionsData | null>(null);
  const [performanceInsightActions, setPerformanceInsightActions] =
    useState<PerformanceInsightActionsData | null>(null);
  const [performanceGoals, setPerformanceGoals] = useState<PerformanceGoalsData | null>(null);
  const [hierarchyDestination, setHierarchyDestination] = useState<string | null>(null);
  const [goalsBusySuggestionId, setGoalsBusySuggestionId] = useState<string | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    const [r1, r1_7, r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, r12, r13] = await Promise.allSettled([
      ...
      fetchPerformanceWorks({ range: performanceRange, limit: 20 }),
      fetchCreativeWorkBundleSuggestions(),
      fetchPerformanceInsightActions({ range: performanceRange }),
      fetchPerformanceGoals({ range: performanceRange })
    ]);
    ...
    if (r5.status === "fulfilled") {
      setUnifiedPerformance(r5.value);
    } else {
      setUnifiedPerformance(null);
      setErrUnifiedPerformance(
        r5.reason instanceof Error ? r5.reason.message : String(r5.reason)
      );
    }

    if (r6.status === "fulfilled") {
      setUsagePreview(r6.value);
    } else {
    ...
    if (r7.status === "fulfilled") {
      setPerformanceOverview(r7.value);
    } else {
      setPerformanceOverview(null);
    }

    if (r8.status === "fulfilled") {
      setPerformanceCampaigns(r8.value);
    } else {
      setPerformanceCampaigns(null);
    }

    if (r9.status === "fulfilled") {
      setPerformanceTags(r9.value);
    } else {
      setPerformanceTags(null);
    }

    if (r10.status === "fulfilled") {
      setPerformanceWorks(r10.value);
    } else {
      setPerformanceWorks(null);
    }

    if (r11.status === "fulfilled") {
      setBundleSuggestions(r11.value);
    } else {
      setBundleSuggestions(null);
    }

    if (r12.status === "fulfilled") {
      setPerformanceInsightActions(r12.value);
    } else {
      setPerformanceInsightActions(null);
    }

    if (r13.status === "fulfilled") {
      setPerformanceGoals(r13.value);
    } else {
      setPerformanceGoals(null);
    }

    const hierarchyFailed = [r7, r8, r9, r10, r12, r13].some((result) => result.status === "rejected");
    if (hierarchyFailed) {
      const firstFailure = [r7, r8, r9, r10, r12, r13].find((result) => result.status === "rejected");
      setErrPerformanceHierarchy(
        firstFailure && firstFailure.status === "rejected"
          ? firstFailure.reason instanceof Error
            ? firstFailure.reason.message
            : String(firstFailure.reason)
          : "Performance hierarchy unavailable."
      );
    }
  ...
  const handlePerformanceRangeChange = useCallback((range: CreatorUnifiedPerformanceRange) => {
    ...
    setErrUnifiedPerformance(null);
    void fetchCreatorUnifiedPerformance({ range, topPostsLimit: 20 })
      .then(setUnifiedPerformance)
      .catch((reason: unknown) => {
        setUnifiedPerformance(null);
        setErrUnifiedPerformance(reason instanceof Error ? reason.message : String(reason));
      });
    void Promise.allSettled([
      fetchPerformanceOverview({ range, destination: hierarchyDestination }),
      fetchPerformanceCampaignRollups({ range }),
      fetchPerformanceTagRollups({ range }),
      fetchPerformanceWorks({ range, limit: 20 }),
      fetchPerformanceInsightActions({ range }),
      fetchPerformanceGoals({ range })
    ]).then(([overview, campaigns, tags, worksList, insightActions, goals]) => {
      if (overview.status === "fulfilled") setPerformanceOverview(overview.value);
      if (campaigns.status === "fulfilled") setPerformanceCampaigns(campaigns.value);
      if (tags.status === "fulfilled") setPerformanceTags(tags.value);
      if (worksList.status === "fulfilled") setPerformanceWorks(worksList.value);
      if (insightActions.status === "fulfilled") setPerformanceInsightActions(insightActions.value);
      if (goals.status === "fulfilled") setPerformanceGoals(goals.value);
    });
  }, [hierarchyDestination]);

  const handleAdoptPerformanceGoalSuggestion = useCallback(
    async (suggestion: PerformanceGoalSuggestionWire) => {
      setGoalsBusySuggestionId(suggestion.suggestion_id);
      try {
        await createPerformanceGoal({
          scope: suggestion.scope,
          scope_ref: suggestion.scope_ref,
          metric: suggestion.metric,
          target_value: suggestion.target_value,
          range: suggestion.range,
          label: suggestion.label
        });
        const refreshed = await fetchPerformanceGoals({ range: performanceRange });
        setPerformanceGoals(refreshed);
      } catch {
        // Keep UI stable; user can retry from suggested list.
      } finally {
        setGoalsBusySuggestionId(null);
      }
    },
    [performanceRange]
  );

  const handleRemovePerformanceGoal = useCallback(async (goalId: string) => {
    try {
      await deletePerformanceGoal(goalId);
      setPerformanceGoals((current) =>
        current
          ? {
              ...current,
      ...
      // Ignore delete failures in UI shell.
    }
  }, []);

  const handleHierarchyDestinationChange = useCallback((destination: string | null) => {
    setHierarchyDestination(destination);
    void fetchPerformanceOverview({ range: performanceRange, destination })
      .then(setPerformanceOverview)
      .catch(() => {
        setPerformanceOverview(null);
      });
  }, [performanceRange]);
  ...
  const legacyActionCards = useMemo(
    () => buildCreatorActionCards({ performance, summary7d, stickiness }),
    [performance, summary7d, stickiness]
  );
  const creatorActionCards = useMemo(
    () => mergeInsightActionCards(legacyActionCards, performanceInsightActions),
    [legacyActionCards, performanceInsightActions]
  );
      ...
      {!loading ? (
        <>
          ...
          {errPerformanceHierarchy ? (
            <p
              className="rounded-2xl border border-[#5c4a2a] bg-[#1a1608] px-3 py-2 text-sm text-[#e8d9a8]"
              data-testid="analytics-hierarchy-error"
            >
              Performance hierarchy partially unavailable: {errPerformanceHierarchy}
            </p>
          ) : null}

          {errUnifiedPerformance ? (
            <p
              className="rounded-2xl border border-[#5c2a2a] bg-[#1a1010] px-3 py-2 text-sm text-[#f0a8a8]"
              data-testid="analytics-unified-error"
            >
              Unified post performance unavailable: {errUnifiedPerformance}
            </p>
          ) : null}
          ...
          {insightsImportStale ? (
            <div
              ...
              className="rounded-2xl border border-[#6a5a2a] bg-[#1a1808] px-3 py-2.5 text-sm leading-snug text-[#e8d9a8]"
            >
          ...
          <details className="group rounded-3xl border border-[#1F1F1F] bg-[#0D0D0D] p-4">
          ...
          >
            <div className="mb-4 flex flex-col gap-3 rounded-2xl border border-dashed border-[#333] bg-[#0A0A0A] p-3 sm:flex-row sm:items-end">
              <div className="flex-1">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <a
                    href="https://www.patreon.com/insights/posts"
                    target="_blank"
                    rel="noreferrer"
                    className="text-xs font-medium text-[#9bf0c4] underline-offset-4 hover:text-[#b8f5d9] hover:underline"
                  >
                    Open Patreon Post Insights
                  </a>
                </div>
                <input
                  id="insights-csv"
                  ...
                  className="mt-1 block w-full text-xs text-[#CCC] file:mr-3 file:rounded-full file:border file:border-[#333] file:bg-[#1a1a1a] file:px-3 file:py-1.5 file:text-xs file:text-[#E0E0E0]"
                  onChange={(e) => setCsvFile(e.target.files?.[0] ?? null)}
                />
                <p className="mt-1 text-[11px] leading-relaxed text-[#666]">
                  Patreon documents post-level CSV export from the Post Insights dashboard via
                  <span className="font-medium text-[#888]"> Export as CSV</span> at the bottom of the table.
                </p>
              </div>
              <button
                type="button"
                ...
                className="rounded-full bg-[#0D3D2C] px-4 py-2 text-sm font-semibold text-[#9bf0c4] disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <footer className="rounded-2xl border border-[#1F1F1F] bg-[#080808] p-4 text-xs leading-relaxed text-[#666]">
