...
function PayloadPreview({ record }: { record: NotificationRecord }): React.ReactElement | null {
  ...
  switch (record.kind) {
    ...
    case "post_collected": {
      ...
      return (
        <p className="mt-1 text-[11px] text-[#888]">
          {post ? <>On post <code className="text-[#bbb]">{post}</code></> : "On your content"}
        </p>
      );
    }
    case "new_subscriber": {
      const tiers = (payload.next_tier_ids as string[] | undefined) ?? [];
      return (
        <p className="mt-1 text-[11px] text-[#888]">
          {tiers.length > 0 ? `Tiers: ${tiers.join(", ")}` : "New active subscriber"}
        </p>
      );
    }
    default:
...
function summarize(record: NotificationRecord): string {
  switch (record.kind) {
    ...
    case "post_commented":
      return record.clusterCount > 1
        ? `${record.clusterCount} new comments on your post`
        : "New comment on your post";
    case "post_favorited":
      return record.clusterCount > 1
        ? `${record.clusterCount} new favorites on your content`
        : "Someone favorited your content";
    case "post_collected":
      return record.clusterCount > 1
        ? `${record.clusterCount} new saves to collections`
