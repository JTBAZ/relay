 ...
 *   the API already sanitizes on write/read. Keep config in sync with
 *   `src/security/post-html-sanitize-config.ts` and `web/_shared/security/post-html-sanitize-config.ts`.
 */

import DOMPurify from "dompurify";
import { POST_HTML_SANITIZE_CONFIG } from "../_shared/security/post-html-sanitize-config";
