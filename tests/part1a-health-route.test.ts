...
describe("GET /api/v1/health/part1a", () => {
  it("returns Part 1 A gate envelope", async () => {
    const tempDir = await mkdtemp(join(tmpdir(), "relay-p1a-"));
    const { app } = createApp(baseConfig(tempDir));

    const res = await request(app).get("/api/v1/health/part1a");
