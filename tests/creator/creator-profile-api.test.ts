...
describe("GET /api/v1/creator/profile", () => {
  it("503 when file identity (no DB)", async () => {
    const tempDir = await mkdtemp(join(tmpdir(), "relay-creator-profile-"));
    const { app } = fileIdentityApp(tempDir);
    const res = await request(app).get("/api/v1/creator/profile");
...
describe("GET /api/v1/public/creators/:slug", () => {
  it("503 when file identity (no DB)", async () => {
    const tempDir = await mkdtemp(join(tmpdir(), "relay-public-creator-"));
    const { app } = fileIdentityApp(tempDir);
    const res = await request(app).get("/api/v1/public/creators/nonexistent-slug");
    expect(res.status).toBe(503);
  });
});
