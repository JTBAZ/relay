...
export default function LibraryPowerPanel({
  ...
  return (
    <aside
      className={[
        "absolute inset-y-0 right-0 z-50 flex min-h-0 w-[min(21rem,calc(100vw-1rem))] shrink-0 flex-col border-l border-[var(--lib-border)] bg-[var(--lib-card)] shadow-2xl shadow-black/50 transition-transform duration-200 ease-out",
        isOpen ? "translate-x-0" : "pointer-events-none translate-x-[calc(100%+1rem)]"
      ].join(" ")}
      aria-hidden={!isOpen}
    >
      {/* Header with mode tabs */}
      <div className="shrink-0 border-b border-[var(--lib-border)] bg-[var(--lib-bg)] p-4">
        <div className="flex items-center justify-between gap-3">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-[var(--lib-fg-muted)]">Power Panel</p>
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-1.5 text-[var(--lib-fg-muted)] transition-colors hover:bg-[var(--lib-muted)] hover:text-[var(--lib-fg)]"
            aria-label="Close power panel"
          >
            <X className="h-4 w-4" aria-hidden />
          </button>
        </div>
        <div className="mt-3 grid grid-cols-4 gap-1 rounded-xl border border-[var(--lib-border)] bg-[var(--lib-card)] p-1">
          {MODES.map((entry) => (
            <button
              key={entry.id}
              type="button"
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
        {/* Preview / Pulse section */}
        <section className="rounded-2xl border border-[var(--lib-border)] bg-[var(--lib-bg)] p-4">
          <div className="mb-3 flex items-center justify-between gap-2">
            ...
            {selectedItems.length > 0 ? (
              <button
                ...
                className="rounded-lg px-2 py-0.5 text-[11px] font-medium text-[var(--lib-fg-muted)] transition-colors hover:bg-[var(--lib-muted)] hover:text-[var(--lib-fg)]"
              >
                Clear
              </button>
            ) : null}
          </div>
          <div className="relative flex h-44 overflow-hidden rounded-xl border border-[var(--lib-border)] bg-[var(--lib-card)]">
            {selectedItem ? (
              <>
              ...
              <div className="flex h-full w-full flex-col items-center justify-center px-6 text-center">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-dashed border-[var(--lib-border)] bg-[var(--lib-muted)]">
                  <Images className="h-5 w-5 text-[var(--lib-fg-muted)]" aria-hidden />
                </div>
                <p className="mt-3 text-sm font-medium text-[var(--lib-fg)]">No post selected</p>
                <p className="mt-1 text-xs leading-relaxed text-[var(--lib-fg-muted)]">
                  Click an item in the gallery to inspect it here.
                </p>
              </div>
            )}
          </div>
