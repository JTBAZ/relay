...
import type { CSSProperties, MouseEvent as ReactMouseEvent, WheelEvent } from "react";
...
type Props = {
  ...
  onToggleCheckbox: (items: GalleryItem[]) => void;
...
export default function GalleryGridTile({
  ...
  // Disable page scroll when hotspot is active
  useEffect(() => {
    if (hotspotActive && multi) {
  ...
  return (
    <div
      data-gallery-tile
      role="listitem"
      className={`group flex h-full min-h-0 w-full min-w-0 flex-col overflow-hidden rounded-2xl bg-[var(--lib-tile)] outline-none transition-all duration-200 ${borderRing} ${keyboardFocusRingClass} ${hovered ? "scale-[1.028] z-10 shadow-xl shadow-black/40" : "scale-100 z-0"}`}
      style={borderColorStyle}
      tabIndex={0}
      onClick={() => {
        if (selectionMode) {
          // In selection mode, any click on the tile selects it
          onToggleSelect(items)
        }
      }}
      onFocus={() => onFocusIndex(flatIndex)}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => { setHovered(false); scrollAccumRef.current = 0; }}
      onWheel={handleWheel}
      onKeyDown={(e) => {
      ...
      <div className={thumbShell}>
        {items.length === 1 && !relayPipelineReady(item) ? (
          ...
          <>
            ...
            {/* Hotspot — small circular badge, bottom-right corner */}
            <div
              className="absolute bottom-2.5 right-2.5 z-[20]"
              onMouseEnter={() => setHotspotActive(true)}
              onMouseLeave={() => { setHotspotActive(false); scrollAccumRef.current = 0; }}
              onWheel={(e) => {
                if (!multi) return;
                e.preventDefault();
                e.stopPropagation();
            ...
            >
              {/* The pill badge — always visible, very subtle */}
              <div
              ...
              >
                <span className={`h-1 w-1 rounded-full transition-colors duration-200 ${hotspotActive ? "bg-[#00aa6f]" : "bg-white/30"}`} aria-hidden />
                <span className={`text-[9px] font-medium leading-none tabular-nums transition-colors duration-200 ${hotspotActive ? "text-white/80" : "text-white/30"}`}>
                  {items.length}
                </span>
              </div>

              {/* Sliding dot-strip — appears above the hotspot on hover */}
              <div
                className={`absolute bottom-full right-0 mb-1.5 transition-all duration-200 ${
                  hotspotActive ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 translate-y-1 pointer-events-none"
                }`}
              >
                <div className="flex items-center gap-1 rounded-full border border-white/10 bg-black/75 px-2 py-1.5 backdrop-blur-md shadow-lg shadow-black/40">
                  {items.map((_, i) => (
                    <button
                      key={i}
                      type="button"
            ...
            {/* Image slide transition layer — swaps on carousel change */}
            <div
              className="absolute inset-0 z-[1] overflow-hidden rounded-xl"
              style={{ transition: "opacity 180ms ease" }}
            >
              {/* Render all images, translate the active one into view */}
              {items.map((it, i) => {
                const vis = postCarouselMainVisual(it);
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      transform: isActive ? "translateX(0)" : `translateX(${i < carouselIdx ? "-8%" : "8%"})`,
                    }}
                  >
                    {vis.relayProcessing ? (
                      <div className="flex h-full w-full items-center justify-center bg-[var(--lib-muted)] px-4 text-center">
                        <span className="text-[11px] font-medium leading-tight text-[var(--lib-fg-muted)]">
                          {relayMediaPlaceholderLabel(it)}
                        </span>
                      </div>
                    ) : vis.src && vis.isVideo ? (
                      <video className="block h-full w-full object-cover object-center" src={vis.src} muted playsInline preload="metadata" aria-hidden />
                    ) : vis.src ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={vis.src} alt="" className="block h-full w-full object-cover object-center" />
                    ) : (
                      <div className="flex h-full w-full items-center justify-center bg-[var(--lib-muted)]">
                        <span className="text-[10px] uppercase tracking-wider text-[var(--lib-fg-muted)]">
                          {mediaTypeLabel(it.mime_type, it.media_id)}
                        </span>
                      </div>
                    )}
                  </div>
        ...
        {showSelectCheckbox ? (
          ...
          >
            <input
              type="checkbox"
              checked={selected}
              onChange={() => onToggleCheckbox(items)}
              onClick={(e) => e.stopPropagation()}
