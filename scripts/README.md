...
- **Commands:** `npm run autopipeline -- status` · `npm run autopipeline -- prepare` · `npm run autopipeline -- complete --taskKey T-006 --exitCode 0 --deltaOutFile ./delta.md` · `npm run autopipeline:run-until-t011:dry` (dry-run) · `npm run autopipeline -- run-until-t011` (loops agent until **T-011** barrier). **PowerShell:** `npm` drops `--flags` after the first script arg unless you insert a second `--` (e.g. `npm run autopipeline -- -- run-until-t011 --dry-run`).
