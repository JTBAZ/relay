...
### 2026-07-27 (ENV-4 dual-read + PAT/FEED browser)

| Check | Agent finding | Verdict |
| ----- | ------------- | ------- |
| **ENV-4 ingress** | Platform webhook previously 404’d (`Unknown webhook delivery token`) because delivery reads the file store while Coolify volume lacked the opaque token; Postgres `webhook_endpoints` still had the dual-write row. Shipped dual-read in `e34cc51` (`PatreonWebhookMetadataStore` → `WebhookEndpoint`). Probe with bad HMAC now returns **401 Invalid Patreon webhook signature** (token+secret resolved). Coolify log: `POST .../webhooks/patreon/platform/<token>` **401**. | Route **PASS**; live Patreon delivery still **human** (tick ENV-4 after one real member/post event) |
| **CORS** | Set `RELAY_ALLOWED_WEB_ORIGINS=https://relayapp.me,https://www.relayapp.me` on Coolify API (literal quotes initially broke allowlist; fixed + restart). Preflight now `ACO=https://relayapp.me` + credentials. Parser also strips Coolify quote wrappers. | Credentialed web→API **PASS** |
| **FEED-1 / FEED-2** | Riley password login via API (`patron_dev_riley@pilot.relay.test`); browser session on `https://relayapp.me/patron/feed`. Sidebar: Dev Ava + Dev Milo (+ Dev Quinn). Feed shows ≥5 posts from followed creators (chronological). API: `GET /patron/follows` **200** (3), `GET /patron/feed?limit=50` **200** (8 posts). Note: prod `/login/pilot-ux` is **404** (stale web build); used API session bootstrap. | Browser **PASS** for seed Riley feed path |
| **Host disk** | Coolify Redis logged `MISCONF ... No space left on device` during scheduler registration (workers still running). | **Ops risk** — free disk on Coolify host |

**Next action (human):** Trigger one Patreon members/posts webhook for ENV-4 live proof; accept FEED-1/FEED-2; continue PAT-1/2 OAuth walls as needed. Prefer freeing VPS disk before more force deploys.
...
| **ENV-4**                             | See newer log section — dual-read shipped; live Patreon event still open                                                                                                                                                                                                     | Ingress fixed; live delivery open                                                                  |
...
| **PAT-2** | Patron Patreon OAuth flow    | 1. After Relay account exists 2. Open /connect/patreon/patron/connect 3. Approve on Patreon 4. Return to app | Memberships via POST /api/v1/auth/patreon/patron/link (replay-safe); Enable Studio is separate | Network: /patron/link 200 | ☐        |
| **PAT-3** | Patron session persistence   | 1. Log in as patron (email/password or OAuth) 2. Refresh page 3. Navigate to `/patron/feed` | Session persists; no re-login needed | Browser DevTools: `relay_session` / `relay_signed_in`; `/me/session` 200 | ☐        |
...
| #          | Scenario                    | Steps                                                                                                                                                                                                         | Expected Result                                                   | Evidence                                                                    | Sign-off |
| ---------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------------------------- | -------- |
| **FEED-1** | Patron follow creators      | 1. Log in as patron (Riley) 2. Navigate to `/patron/library` 3. Search for "Ava" → click follow 4. Repeat for "Milo"                                                                                          | Patron now follows both; sidebar updates                          | Seed Riley already follows Ava+Milo (+Quinn); browser sidebar + `GET /api/v1/patron/follows` 200 (3 rows) 2026-07-27 | ☑        |
| **FEED-2** | Feed fetches followed posts | 1. After FEED-1 follow setup 2. Navigate to `/patron/feed` 3. Wait 2s for load                                                                                                                                | Feed displays posts from Ava + Milo sorted by date (newest first) | Browser `/patron/feed` as Riley; `GET /api/v1/patron/feed?limit=50` 200 with 8 posts (Ava/Milo/Quinn) 2026-07-27 | ☑        |
