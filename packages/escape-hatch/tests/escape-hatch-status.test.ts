...
describe("buildEscapeHatchStatus", () => {
  ...
  it("documents private media layout and non-authoritative persona state", () => {
    ...
    expect(media?.nextSlice).toBe("EH-041");
  });

  it("records EH-040 complete and routes next work to EH-041", () => {
    const status = buildEscapeHatchStatus();
    ...
    expect(status.nextSlice.title).toMatch(/Relay-managed|verification/i);
  ...
  it("reports shared contracts and aligned preview access honestly", () => {
    ...
    expect(access?.nextSlice).toBe("EH-041");
  });
  ...
  it("routes capability ownership to the corrected batting-order slices", () => {
    ...
    expect(capabilities.get("private-media-delivery")?.nextSlice).toBe("EH-041");
    expect(capabilities.get("account-paywall-ux")?.nextSlice).toBe("EH-041");
    ...
    expect(capabilities.get("provider-readiness")?.nextSlice).toBe("EH-041");
    ...
    expect(capabilities.get("generated-site-identity")?.nextSlice).toBe("EH-041");
    expect(capabilities.get("entitlement-evaluator")?.state).toBe("preview_only");
    expect(capabilities.get("entitlement-evaluator")?.nextSlice).toBe("EH-041");
  });
  ...
  it("cites wired matrix, scan, and real-shape Patreon fixture paths", () => {
    ...
    expect(capability?.nextSlice).toBe("EH-041");
  ...
  it("marks migration-import and relay-dump as implemented preview capabilities", () => {
    ...
    expect(migration?.nextSlice).toBe("EH-041");
    ...
    expect(dump?.nextSlice).toBe("EH-041");
  });
