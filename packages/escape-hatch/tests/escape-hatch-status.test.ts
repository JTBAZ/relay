...
const EXPECTED_CAPABILITY_IDS = [
  ...
  "relay-managed-connector-billing",
  "oauth-choice-migration-ux"
] as const;
...
describe("buildEscapeHatchStatus", () => {
  ...
  it("records EH-082 local QC accepted and routes next work to HUMAN-SIGNOFF", () => {
    const status = buildEscapeHatchStatus();
    ...
    expect(
      status.capabilities.some((c) => c.id === "oauth-choice-migration-ux")
    ).toBe(true);
  ...
  it("reports shared contracts and aligned preview access honestly", () => {
    ...
    expect(access?.nextSlice).toBe("HUMAN-SIGNOFF");
  });
  ...
  it("routes capability ownership to the corrected batting-order slices", () => {
    ...
    expect(capabilities.get("private-media-delivery")?.nextSlice).toBe("HUMAN-SIGNOFF");
    expect(capabilities.get("account-paywall-ux")?.nextSlice).toBe("HUMAN-SIGNOFF");
    expect(capabilities.get("billing-adapters")?.nextSlice).toBe("HUMAN-SIGNOFF");
    expect(capabilities.get("deploy-adapters")?.nextSlice).toBe("HUMAN-SIGNOFF");
    expect(capabilities.get("backup-restore")?.nextSlice).toBe("HUMAN-SIGNOFF");
    expect(capabilities.get("provider-readiness")?.nextSlice).toBe("HUMAN-SIGNOFF");
    expect(capabilities.get("provider-readiness")?.evidence).toMatch(
      /EH-030|EH-031|Auth\/DB|EH-051|EH-070|EH-072|EH-073/
    );
    expect(capabilities.get("generated-site-identity")?.state).toBe("preview_only");
    expect(capabilities.get("generated-site-identity")?.nextSlice).toBe("HUMAN-SIGNOFF");
    expect(capabilities.get("entitlement-evaluator")?.state).toBe("preview_only");
    expect(capabilities.get("entitlement-evaluator")?.nextSlice).toBe("HUMAN-SIGNOFF");
  });
  ...
  it("cites wired matrix, scan, and real-shape Patreon fixture paths", () => {
    ...
    expect(capability?.nextSlice).toBe("HUMAN-SIGNOFF");
  ...
  it("marks migration-import and relay-dump as implemented preview capabilities", () => {
    ...
    expect(migration?.nextSlice).toBe("HUMAN-SIGNOFF");
    ...
    expect(dump?.nextSlice).toBe("HUMAN-SIGNOFF");
  });
