...
- **Creator-owned Patreon OAuth (EH-040)** uses **your** Patreon client credentials and **your** `ESCAPE_HATCH_PATREON_TOKEN_KEY`. Refresh tokens are encrypted at rest on the site; they must not appear in generated zips, browser bundles, logs, diagnostic packets, or Relay records after handoff. **Relay-managed verification (EH-041)** is an optional alternative: Relay returns short-lived assertions; the site does not hold Patreon tokens in that mode.
- **Transactional email (EH-072)**, **backup/restore (EH-073)**, and **launch wizard (EH-074)** are creator-operated kit surfaces. Generate the **ownership packet (EH-080)** from `/admin/deploy` for manifesto, env-name inventory, optional Relay disclosure, and 90-day warranty boundary — **never** embed secrets.
- Relay optional services (Crosspost, managed Patreon) are disclosed, revocable, and replaceable; they are not required to build or operate this chassis natively.
- This kit is **`productionSafe: false`**: EH-082 local security/independence QC is accepted; **HUMAN-SIGNOFF** and live-provider proofs remain open. Independence packet stamp is `local_native_passed_live_provider_open` (not live provider independence). `public_legacy` media mode is blocked for premium delivery and refused at fill time — do not enable it.

Bootstrap, Path A vs Path B, entitlement freshness, R2 private bucket setup, Patreon OAuth setup/rotation, account UX, and key/password rotation: `scripts/bootstrap-identity.md` and `OPERATIONS.md`. See `escape-hatch.manifest.json` for chassis/schema versions and adapter states. Packet download: `data/ownership-packet/` after generate.
