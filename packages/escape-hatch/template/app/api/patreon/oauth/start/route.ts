...
/**
 * Start creator-owned Patreon OAuth (EH-040).
 * POST only (HTTP verb hygiene + account-linking CSRF: SameSite=Lax sessions
 * would otherwise allow cross-site GET to mint a victim-bound authorize URL).
 */
export async function POST(request: Request): Promise<NextResponse> {
  ...
  if (!isSameOriginOAuthStart(request)) {
    return NextResponse.json(
      {
        ok: false,
        error: "Cross-origin OAuth start rejected.",
...
export async function GET() {
  return NextResponse.json(
    {
      ok: false,
      error: "Patreon OAuth start requires POST (CSRF / verb hygiene).",
      production_safe: false
    },
